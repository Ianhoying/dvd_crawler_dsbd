...
with tab1:
	...
	st.dataframe(div_cut, hide_index = True, width = 2000, height = 500)

	st.subheader('배당내역 불일치 종목\n\n')
	st.caption('')
	st.dataframe(div_check, hide_index = True, width = 2000, height = 500)
	
	st.subheader('주식 분할/병합 예정 종목\n\n')
	st.caption('')
	st.dataframe(total_splits, hide_index = True, width = 2000, height = 500)
